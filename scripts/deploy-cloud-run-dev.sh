...
INFERENCE_BACKEND="${INFERENCE_BACKEND:-declip}"
DECLIP_ARTIFACT_URI="${DECLIP_ARTIFACT_URI:-gs://anthonymleedom-declip-v2-dev-models/declip-model-artifacts/declip_state_dict_v1/declip/tgram-deftan2-v005-48khz/20260605T224918Z/}"
DECLIP_DEVICE="${DECLIP_DEVICE:-cpu}"
DECLIP_EXPECTED_ARTIFACT_FORMAT="${DECLIP_EXPECTED_ARTIFACT_FORMAT:-declip_state_dict_v1}"
DECLIP_EXPECTED_FORMAT_VERSION="${DECLIP_EXPECTED_FORMAT_VERSION:-1}"
DECLIP_EXPECTED_MODEL_FAMILY="${DECLIP_EXPECTED_MODEL_FAMILY:-declip}"
DECLIP_EXPECTED_MODEL_NAME="${DECLIP_EXPECTED_MODEL_NAME:-tgram-deftan2-v005-48khz}"
DECLIP_EXPECTED_MODEL_VERSION="${DECLIP_EXPECTED_MODEL_VERSION:-20260605T224918Z}"
DECLIP_EXPECTED_DECLIP_MODEL_VERSION="${DECLIP_EXPECTED_DECLIP_MODEL_VERSION:-0.1.1}"
DECLIP_EXPECTED_TRAINING_GIT_REVISION="${DECLIP_EXPECTED_TRAINING_GIT_REVISION:-}"
DECLIP_REJECT_DIRTY_TRAINING="${DECLIP_REJECT_DIRTY_TRAINING:-true}"
...
if [[ "${APP_RUNTIME_MODE}" == "cloud" ]]; then
  ...
  if [[ "${INFERENCE_BACKEND}" == "declip" ]]; then
    [[ -z "${DECLIP_ARTIFACT_URI}" ]] && missing+=("DECLIP_ARTIFACT_URI")
  fi
  if (( ${#missing[@]} > 0 )); then
...
gcloud run deploy "${SERVICE}" \
  ...
  --set-env-vars "^|^APP_ENV=${APP_ENV}|APP_RUNTIME_MODE=${APP_RUNTIME_MODE}|APP_NAME=${APP_NAME}|APP_VERSION=${APP_VERSION}|LOG_LEVEL=${LOG_LEVEL}|GCP_PROJECT_ID=${GCP_PROJECT_ID}|FIRESTORE_DATABASE=${FIRESTORE_DATABASE}|GCS_BUCKET_NAME=${GCS_BUCKET_NAME}|CLOUD_TASKS_CONVERSION_QUEUE=${CLOUD_TASKS_CONVERSION_QUEUE}|CLOUD_TASKS_LOCATION=${CLOUD_TASKS_LOCATION}|CLOUD_TASKS_SERVICE_ACCOUNT=${CLOUD_TASKS_SERVICE_ACCOUNT}|CONVERSION_SERVICE_URL=${CONVERSION_SERVICE_URL}|CONVERSION_SERVICE_AUDIENCE=${CONVERSION_SERVICE_AUDIENCE}|INFERENCE_SERVICE_AUDIENCE=${INFERENCE_SERVICE_AUDIENCE}|ALLOWED_INTERNAL_CALLER_SERVICE_ACCOUNTS=${allowed_internal_caller_service_accounts}|MODEL_CONFIG_PATH=${MODEL_CONFIG_PATH}|APP_CONFIG_PATH=${APP_CONFIG_PATH}|MODEL_ARTIFACT_CACHE_DIR=${MODEL_ARTIFACT_CACHE_DIR}|INFERENCE_DEVICE=${INFERENCE_DEVICE}|INFERENCE_BACKEND=${INFERENCE_BACKEND}|DECLIP_ARTIFACT_URI=${DECLIP_ARTIFACT_URI}|DECLIP_DEVICE=${DECLIP_DEVICE}|DECLIP_EXPECTED_ARTIFACT_FORMAT=${DECLIP_EXPECTED_ARTIFACT_FORMAT}|DECLIP_EXPECTED_FORMAT_VERSION=${DECLIP_EXPECTED_FORMAT_VERSION}|DECLIP_EXPECTED_MODEL_FAMILY=${DECLIP_EXPECTED_MODEL_FAMILY}|DECLIP_EXPECTED_MODEL_NAME=${DECLIP_EXPECTED_MODEL_NAME}|DECLIP_EXPECTED_MODEL_VERSION=${DECLIP_EXPECTED_MODEL_VERSION}|DECLIP_EXPECTED_DECLIP_MODEL_VERSION=${DECLIP_EXPECTED_DECLIP_MODEL_VERSION}|DECLIP_EXPECTED_TRAINING_GIT_REVISION=${DECLIP_EXPECTED_TRAINING_GIT_REVISION}|DECLIP_REJECT_DIRTY_TRAINING=${DECLIP_REJECT_DIRTY_TRAINING}|MAX_DECODED_DURATION_SECONDS=${MAX_DECODED_DURATION_SECONDS}"
